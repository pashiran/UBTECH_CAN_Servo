...
#### 송신 프레임
```
AA 00 00 01 00 00 00 [ID] 01 00 00 00 00 00 00 00
```

| 필드 | 값 | 설명 |
|------|-----|------|
| 바이트 0-2 | AA 00 00 | 프레임 헤더 |
| 바이트 3 | 0x01 | 데이터 길이 |
| 바이트 7 | ID | 0x00=전체, 또는 특정 ID |
| 바이트 8 | 0x01 | 명령 코드 |
| 바이트 9-15 | 0x00 | 파라미터 없음 |

#### 수신 프레임
```
AA 00 00 08 00 00 00 [ID] 02 [POS_L] [POS_H] [HW_VER] [SW_VER] XX XX XX
```
...
| 바이트 7 | 응답한 서보 ID |
| 바이트 8 | 0x02 (응답 코드) |
| 바이트 9-10 | 현재 위치 (Little-Endian, 0-4095) |
| 바이트 11 | 하드웨어 버전 |
| 바이트 12 | 소프트웨어 버전 |
...
**ID 13인 서보 조회**
```
송신: AA 00 00 01 00 00 00 0D 01 00 00 00 00 00 00 00
수신: AA 00 00 08 00 00 00 0D 02 03 08 62 60 17 10 10
```
- 위치: 0x0803 = 2051 (리틀 엔디안) → 약 180.3도
- HW 버전: 0x62
- SW 버전: 0x60
...
#### 송신 프레임
```
AA 00 00 05 00 00 00 [ID] 03 [POS_L] [POS_H] [SPD_L] [SPD_H] 00 00 00
```
...
| 바이트 7 | ID | 대상 서보 ID |
| 바이트 8 | 0x03 | 명령 코드 |
| 바이트 9-10 | 위치 | 목표 각도 (0-4095) |
| 바이트 11-12 | 속도 | 이동 속도 (0-280 deg/s) |
| 바이트 13-15 | 0x00 | 예약 |

#### 수신 프레임
```
AA 00 00 03 00 00 00 [ID] 03 [STATUS] AA [POS_L] [POS_H] 00 00 00
```

| 필드 | 설명 |
|------|------|
| 바이트 9 | 상태 코드 (0x04=실행중, 0xAA=완료) |
| 바이트 10 | 0xAA (정상) |
| 바이트 11-12 | 현재 위치 |
...
#### 예제
**ID 13인 서보를 180도로 150deg/s 속도로 이동**
```
송신: AA 00 00 05 00 00 00 0D 03 00 08 96 00 00 00 00
수신: AA 00 00 03 00 00 00 0D 03 04 AA 96 00 00 00 00
```
- 위치: 0x0800 = 2048 → 180도
- 속도: 0x0096 = 150 deg/s
...
**송신 프레임**
```
AA 00 00 01 00 00 00 [OLD_ID] 07 00 00 00 00 00 00 00
```
...
**송신 프레임**
```
AA 00 00 06 00 00 00 [OLD_ID] 07 [NEW_ID] [CODE1] [CODE2] [CODE3] [CODE4] 00 00
```
...
#### 송신 프레임
```
AA 00 00 01 00 00 00 [ID] 11 00 00 00 00 00 00 00
```
...
#### 송신 프레임
```
AA 00 00 01 00 00 00 [ID] 2F 00 00 00 00 00 00 00
```
